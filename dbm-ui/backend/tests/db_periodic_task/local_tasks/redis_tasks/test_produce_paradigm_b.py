...
def _make_fake_task_cls(
    ...
    *,
    ...
    class _FakeQueue:
        ...
        @classmethod
        def inflight_count(cls):
            return queue_inflight
    ...
    class _FakeTask:
        ...
        def submit(self, items, *, execute_at=None):
            items = list(items)
            submit_recorder.append((items, execute_at))
...
def test_both_lanes_when_priority_due_and_below_watermark(task_mod):
    mod = task_mod
    submits = []
    fake_cls = _make_fake_task_cls(mod, pendings=0, submit_recorder=submits)
    ...
    # Lane 1 (priority) runs first and jumps the queue with an early execute_at.
    prio_items, prio_execute_at = submits[0]
    assert [i["cluster_id"] for i in prio_items] == [1]
    assert isinstance(prio_execute_at, float)

    # Lane 2 (rotation) submits the top-up batch; no spread window -> execute_at None.
    rot_items, rot_execute_at = submits[1]
    assert [i["cluster_id"] for i in rot_items] == [2, 3]
    assert rot_execute_at is None
...
def test_no_lanes_when_priority_done_and_above_watermark(task_mod):
    mod = task_mod
    submits = []
    fake_cls = _make_fake_task_cls(mod, pendings=500, submit_recorder=submits)  # >= low_watermark
...
def test_disabled_task_produces_nothing(task_mod):
    mod = task_mod
    submits = []
    fake_cls = _make_fake_task_cls(mod, pendings=0, submit_recorder=submits, enabled=False)
...
def test_priority_failure_does_not_mark_daily_pass(task_mod):
    mod = task_mod
    submits = []
    fake_cls = _make_fake_task_cls(mod, pendings=500, submit_recorder=submits)
    selector = MagicMock()
...
def test_priority_manifest_consumed_across_beats(task_mod):
    ...
    fake_cls = _make_fake_task_cls(
        mod,
        pendings=500,  # at/above watermark: rotation lane skipped
        submit_recorder=submits,
        queue_pending=3,
        queue_inflight=1,
        max_admitted_jobs=6,
...
def test_priority_manifest_cursor_held_on_capacity_rejection(task_mod):
    ...
    # Admission failure: the cursor must not advance past the initial 0, the
    # manifest must stay, and the pass must stay claimed so the next beat
