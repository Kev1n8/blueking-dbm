...
class TestTruncateAgentResponseForLog:
    ...
    def test_non_string_uses_repr(self, ai_tasks, value):
        from backend.dbm_aiagent.tasks.invoker import truncate_agent_response_for_log

        assert truncate_agent_response_for_log(value) == repr(value)

    def test_short_string_returned_as_repr(self, ai_tasks):
        from backend.dbm_aiagent.tasks.invoker import truncate_agent_response_for_log

        assert truncate_agent_response_for_log("hello") == repr("hello")

    def test_exact_max_chars_not_truncated(self, ai_tasks):
        from backend.dbm_aiagent.tasks.invoker import AGENT_RESPONSE_LOG_MAX_CHARS, truncate_agent_response_for_log

        s = "a" * AGENT_RESPONSE_LOG_MAX_CHARS
        assert truncate_agent_response_for_log(s) == repr(s)

    def test_long_string_truncated_with_total_len_marker(self, ai_tasks):
        from backend.dbm_aiagent.tasks.invoker import AGENT_RESPONSE_LOG_MAX_CHARS, truncate_agent_response_for_log

        s = "a" * (AGENT_RESPONSE_LOG_MAX_CHARS + 500)
        out = truncate_agent_response_for_log(s)
...
class TestAgentCheckTaskFailureHandler:
    ...
    def test_worker_lost_drops_inflight_when_job_exists(self, ai_tasks):
        ...
        with (
            ...
            patch("backend.db_periodic_task.dispatch.metrics.DispatchMetrics.record_task_counter") as record_task,
        ):
            self._fire(
                sender=self._sender(),
    ...
    def test_worker_lost_finalizes_in_job_namespace_when_queue_unregistered(self):
        ...
        with (
            ...
            patch("backend.db_periodic_task.dispatch.metrics.DispatchMetrics.record_task_counter"),
        ):
            self._fire(
                sender=self._sender(),
                exception=WorkerLostError("killed"),
    ...
    def test_hard_time_limit_drops_inflight_when_job_exists(self):
        ...
        # --time-limit kills a task: reclaim the inflight slot immediately.
        queue_cls.record_outcome.assert_called_once_with("dummy.smoke", DispatchOutcomeType.ERROR)
        lifecycle_finalize.assert_called_once_with(
            queue_cls=queue_cls,
            job_id="dummy.smoke:item-1",
...
class TestSkipBeforeInvoke:
    ...
    def test_offline_cluster(self, fake_task_instance, make_cluster, no_recent_tickets):
        ...
        skipped, reason = task.skip_before_invoke(
    ...
    def test_busy_with_recent_or_active_ticket(self, fake_task_instance, make_cluster):
        ...
        with patch(self._ORM) as mod:
            ...
            skipped, reason = task.skip_before_invoke({"cluster_id": make_cluster().id, "cluster": make_cluster()})
...
class TestAgentInvokerOutcomes:
            ...
            *,
            ...
            with patch(self._HANDLER) as handler_cls:
                ...
                return AgentInvoker.invoke(
                    ...
                    invoke_timeout_seconds=config.agent_invoke_timeout_seconds,
                    retry_count=0,
    ...
    def test_api_timeout(self, ai_tasks, invoke, caplog):
        ...
        assert outcome.outcome == ai_tasks.DispatchOutcomeType.TIMEOUT_INVOKE
