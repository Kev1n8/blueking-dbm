...
def rebalance_module(django_db_setup, django_db_blocker):
    with django_db_blocker.unblock():
        return importlib.import_module("backend.db_periodic_task.dispatch.rebalance")
...
def test_sweep_namespace_clears_only_its_own_namespace(rebalance_module):
    ...
    with patch("backend.db_periodic_task.dispatch.routing.conn_for_alias", return_value=client):
        result = rebalance_module.sweep_namespace("dispatch_0", "ns", dry_run=False)
...
def test_sweep_namespace_dry_run_counts_without_deleting(rebalance_module):
    client = _FakeScanClient(["dispatch:ns:pending", "dispatch:ns:job:a"])
    with patch("backend.db_periodic_task.dispatch.routing.conn_for_alias", return_value=client):
        result = rebalance_module.sweep_namespace("dispatch_0", "ns", dry_run=True)
...
@pytest.mark.django_db
def test_remap_order_pause_save_wait_sweep_wait_sweep_no_resume(rebalance_module):
    with override_settings(DISPATCH_REDIS_ALIASES=["dispatch_0", "dispatch_1"]):
        ...
        with patch.object(
            rebalance_module, "pause_queue_pump", side_effect=lambda *args, **kwargs: order.append("pause")
        ) as pause, patch.object(
            rebalance_module, "_wait_convergence_window", side_effect=lambda: order.append("wait")
            ...
            result = rebalance_module.remap_namespace("ns", "dispatch_1", mode="drop", dry_run=False)
...
@pytest.mark.django_db
def test_remap_mutation_runs_full_clean_and_never_queryset_update(rebalance_module):
    with override_settings(DISPATCH_REDIS_ALIASES=["dispatch_0", "dispatch_1"]):
        DispatchQueueRoute.objects.create(
            namespace="ns",
            redis_alias="dispatch_0",
        ...
        with patch.object(rebalance_module, "pause_queue_pump"), patch.object(
            rebalance_module, "_wait_convergence_window"
        ), patch.object(
            rebalance_module, "sweep_namespace", return_value={"matched": 0, "deleted": 0, "batches": 0}
        ), patch.object(
            DispatchQueueRoute, "full_clean"
        ) as full_clean, patch.object(
            DispatchQueueRoute.objects, "update"
        ) as queryset_update:
            rebalance_module.remap_namespace("ns", "dispatch_1", dry_run=False)
...
@pytest.mark.django_db
def test_remap_second_sweep_warns_on_late_keys(rebalance_module, caplog):
    with override_settings(DISPATCH_REDIS_ALIASES=["dispatch_0", "dispatch_1"]):
        DispatchQueueRoute.objects.create(
            namespace="ns",
            redis_alias="dispatch_0",
        ...
        with patch.object(rebalance_module, "pause_queue_pump"), patch.object(
            rebalance_module, "_wait_convergence_window"
        ), patch.object(rebalance_module, "sweep_namespace", side_effect=sweeps), caplog.at_level(
            "ERROR", logger="root"
        ):
            rebalance_module.remap_namespace("ns", "dispatch_1", dry_run=False)
...
def test_fetch_queued_job_fanout_finds_payload_across_candidates():
    ...
    with patch("backend.db_periodic_task.dispatch.base.namespace_for_job_id", return_value=""), patch(
        ...
        assert DispatchTask.fetch_queued_job("ghost.task:item") is job
...
def test_execute_job_unknown_task_cleans_up_via_job_namespace():
    ...
    ghost_queue = MagicMock()
    with patch.object(DispatchTask, "fetch_queued_job", return_value=job), patch.dict(
...
def test_execute_job_missing_payload_unregistered_is_idempotent_noop():
    ...
    with patch.object(DispatchTask, "fetch_queued_job", return_value=None), patch(
...
def test_pump_publishes_wire_format_args_job_id_only(pump_module):
    ...
    with patch.object(
        ...
        pump_module._dispatch_candidates(
            queue,
            [job],
            DispatchQueueConfig(max_inflight=10),
