...
from backend.db_periodic_task.dispatch.scheduling import at_front, resolve_execute_at, spread


class TestSchedulingHelpers:
    def test_resolve_execute_at_none_float_callable(self):
        assert resolve_execute_at(None, 0, "x") is None
        assert resolve_execute_at(123.0, 5, "x") == 123.0
        assert resolve_execute_at(lambda index, item: 100.0 + index, 3, "x") == 103.0
    ...
    def test_spread_is_monotonic_and_within_window(self):
        ...
        values = [resolve_execute_at(schedule, i, None, count=5) for i in range(5)]
...
class TestSubmitExecuteAt:
    ...
    def test_scalar_execute_at_applied_to_all(self):
        task = self._task()
        jobs = self._capture_jobs(task, [1, 2, 3], execute_at=555.0)
        assert [j.execute_at for j in jobs] == [555.0, 555.0, 555.0]

    def test_callable_execute_at_uses_global_index_across_chunks(self):
        task = self._task()
        # 26 items -> two admission chunks (25 + 1); index must stay global.
        jobs = self._capture_jobs(task, list(range(26)), execute_at=lambda index, item: float(index))
        assert [j.execute_at for j in jobs] == [float(i) for i in range(26)]

    def test_spread_binds_total_count_across_chunks(self):
        task = self._task()
        jobs = self._capture_jobs(task, list(range(26)), execute_at=spread(260.0, base=0.0))
        assert [j.execute_at for j in jobs] == [float(i * 10) for i in range(26)]
    ...
    def test_spread_delay_is_added_before_queue_wait_ttl(self):
        ...
        with patch.object(
            ...
            self._task().submit([1, 2, 3], execute_at=spread(30.0, base=100.0))

        args = script.call_args.kwargs["args"]
        snapshots = [json.loads(args[offset + 2]) for offset in (8, 12, 16)]
        assert [snapshot["execute_at"] for snapshot in snapshots] == [100.0, 110.0, 120.0]
        assert [snapshot["queue_deadline_at"] for snapshot in snapshots] == [86500.0, 86510.0, 86520.0]
        assert [args[offset + 3] for offset in (8, 12, 16)] == [86400, 86410, 86420]
    ...
    def test_callable_is_fully_resolved_before_enqueue(self):
        ...
        with patch.object(QueueAdmission, "enqueue_jobs") as enqueue_jobs, pytest.raises(
            ...
            task.submit(list(range(26)), execute_at=schedule)
