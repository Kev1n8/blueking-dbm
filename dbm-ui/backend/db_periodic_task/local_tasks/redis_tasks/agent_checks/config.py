...
from backend.dbm_aiagent.tasks.config import DEFAULT_LOOKBACK_DAYS, AITaskConfig, IdempotenceMode
...
class RedisAgentCheckConfig(AITaskConfig):
    ...
    # Spread rotation execute_at over this window (seconds); 0 = enqueue at now.
    produce_spread_window_seconds: int = 0
    # Priority (alarm) jobs use execute_at = now - lead to jump the pending queue.
