...
def _has_admission_failure(outcomes) -> bool:
    return any(
        outcome.outcome
        in {
...
def _available_admission_slots(task) -> int:
    ...
    return min(
        REDIS_AGENT_CHECK_MAX_PRODUCE_BATCH_SIZE,
        max(0, int(queue_config.max_admitted_jobs) - pending - inflight),
    )
...
def _produce_redis_agent_check(task_cls, *, filter_candidates=None):
    ...
    if pending >= 0 and pending < low and available > 0:
        ...
        if rotation_items:
            ...
            rotation_outcomes = task.submit(rotation_items, execute_at=spread(window) if window > 0 else None)
            enqueued += _count_enqueued(rotation_outcomes)
        if not _has_admission_failure(rotation_outcomes):
            _write_rotation_cursor(task.task_key, next_cursor)
        # Capacity rejection deliberately holds the cursor: the same window is
        # rescanned next beat and dedupe/watermark block duplicate enqueues, so
        # freed capacity is refilled immediately and no cluster is skipped to
        # the next full rotation. The cost is a repeated DB scan per beat while
        # 0 < available < batch — completeness beats scan efficiency here.
