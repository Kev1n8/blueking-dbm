...
local inflight_removed = redis.call('ZREM', KEYS[2], ARGV[1])
...
if ARGV[3] ~= '' and inflight_removed > 0 then
    local value = tonumber(redis.call('HINCRBY', KEYS[3], ARGV[3], -1))
    if value <= 0 then
        redis.call('HDEL', KEYS[3], ARGV[3])
    end
end
if pending_removed > 0 or inflight_removed > 0 then
...
class OrphanReaper:
    """Purge members whose payload is gone or whose wait deadline has expired.
    ...
    @classmethod
    def purge_member(cls, queue_cls: type[DispatchQueue], job_id: str, *, task_key: str = "") -> tuple[int, int]:
        """Remove ``job_id`` from this queue's pending/inflight zsets; adjust counters.
    ...
    def discard_orphaned_job(
        ...
        *,
        ...
        Clears pending/inflight membership, job payload, and dedupe key. Routes to
        ...
        member is purged from every registered queue, but dedupe keys are deleted
        only in the namespaces where the member was actually found — never
        ...
        try:
            ...
            if target_queue is not None:
                pending_removed, inflight_removed = cls.purge_member(
                    target_queue,
                    job_id,
                    task_key=resolved_task_key,
                )
                dedupe_namespaces.add(target_queue._ns())
            else:
                for candidate_queue in queue_cls.iter_queues():
                    pending_hit, inflight_hit = cls.purge_member(
                        candidate_queue,
                        job_id,
                        task_key=resolved_task_key,
                    )
            ...
            if resolved_task_key and touched:
                ...
                outcome_queue.record_outcome(resolved_task_key, outcome, completed=False)
            if not touched:
                return True
        except Exception as exc:
    ...
    @classmethod
    def reap_orphaned_queue_members(
        cls,
        queue_cls: type[DispatchQueue],
        *,
        scan_limit: int = 200,
        ...
        """Incrementally purge members whose payload TTL has expired.
