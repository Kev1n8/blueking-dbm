...
from backend.db_periodic_task.dispatch.queue import KEY_PRODUCER_LOCK_PREFIX
...
gate key exists, ``submit`` returns ``enqueue_producer_rejected`` and nothing is
...
def producer_lock_key(namespace: str) -> str:
    """The producer gate key for one namespace (``dispatch:{ns}:producer_lock``)."""
    return KEY_PRODUCER_LOCK_PREFIX.format(ns=namespace or "")


def pause_queue_producer(namespace: str, *, seconds: Optional[float] = None, alias: Optional[str] = None) -> dict:
    """Close the per-namespace producer gate so ``submit`` returns ``enqueue_producer_rejected``.
...
def inspect_queue_producer_lock(namespace: str, *, alias: Optional[str] = None) -> dict:
    """Inspect who holds ``dispatch:{ns}:producer_lock``.

    Returns::

        {
            "namespace": "...",
            "key": "dispatch:{ns}:producer_lock",
            "held": bool,
            "owner": str | None,          # raw Redis value
            "state": "free" | "paused" | "held",
...
def is_queue_producer_paused(namespace: str, *, alias: Optional[str] = None) -> bool:
    ...
    return inspect_queue_producer_lock(namespace, alias=alias)["state"] == "paused"
...
def queue_producer_pause_ttl(namespace: str, *, alias: Optional[str] = None) -> Optional[int]:
    ...
    """
    info = inspect_queue_producer_lock(namespace, alias=alias)
