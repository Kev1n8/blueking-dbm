...
class DispatchJob:
    ...
    def sample_identity(self) -> str:
        """Stable identity shared by queue-wait and execution sampling."""
        return f"{self.job_id}:{self.created_at:.6f}"
    ...
    def from_dict(cls, raw: dict) -> "DispatchJob":
        return cls(
            ...
            execute_at=float(raw.get("execute_at", 0)),
            queue_deadline_at=float(raw.get("queue_deadline_at", 0)),
        )
...
def compute_wait_deadline(
    ...
    place. ``queue_deadline_at`` 0 (unset) starts the budget at
    ``max(now, execute_at)`` — intentional future scheduling (``spread``) does
    ...
    if not queue_deadline_at:
        deadline = max(now, float(execute_at)) + wait_ttl
        return deadline, math.ceil(deadline - now)
    ttl = min(wait_ttl, math.ceil(queue_deadline_at - now))
    if ttl < 1:
        return queue_deadline_at, None
    return queue_deadline_at, ttl
