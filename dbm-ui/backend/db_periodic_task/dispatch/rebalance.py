...
convergence get ``enqueue_producer_rejected`` instead of enqueueing into a
...
def remap_namespace(
    ...
    *,
    ...
    if late_keys:
        logger.error(
            "dispatch rebalance: late keys after second convergence window "
        ...
        logger.info(
            "dispatch rebalance: remap complete namespace=%s from_alias=%s to_alias=%s "
