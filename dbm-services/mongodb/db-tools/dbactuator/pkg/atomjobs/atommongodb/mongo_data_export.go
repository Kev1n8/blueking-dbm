...
func (s *mongoDataExport) Init(runtime *jobruntime.JobGenericRuntime) error {
	...
	if err != nil {
		return errors.Wrap(err, "get mongodump bin")
	}
	if !util.FileExists(s.MongoDump) {
		return errors.Errorf("mongodump not exists, path: %s", s.MongoDump)
	}

	s.MongoExport, err = pitr.GetMongoExportBin()
	if err != nil {
		return errors.Wrap(err, "get mongoexport bin")
	}
	if !util.FileExists(s.MongoExport) {
		return errors.Errorf("mongoexport not exists, path: %s", s.MongoExport)
	}
